page 50217 "IT GM Inbox Handled G/L Acc."
{
    Caption = 'GM Inbox Handled G/L Account';
    PageType = Card;
    SourceTable = "IT GM Inbox Handled G/L Acc.";
    Editable = false;
    layout
    {
        area(content)
    ...
    actions
    {
        area(Processing)
        {
            action("Dimensions")
            {
                ...
                RunPageLink = "Table ID" = const(15), "No." = field("No."), "Inbox Entry No." = field("Inbox Entry No.");
                trigger OnAction()
                begin
